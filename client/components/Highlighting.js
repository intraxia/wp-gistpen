...
const onThemeChange = R.map(R.pipe(R.path(['target', 'value']), themeChangeAction));
const onLineNumbersChange = R.map(
    R.pipe(R.path(['target', 'checked']), lineNumbersChangeAction)
);
const onShowInvisiblesChange = R.map(
    R.pipe(R.path(['target', 'checked']), showInvisiblesChangeAction)
);

export default ({ stream$ }: ObservableProps<HighlightingProps>) => (
    <Collector>
        <div className="table">
